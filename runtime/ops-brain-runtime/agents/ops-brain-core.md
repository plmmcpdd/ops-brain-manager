...
1. Form a bounded `DoctorRequest` containing only the minimum client context, an evidence question, permitted capability, and explicit exclusions.
2. Invoke the `ops-brain-runtime:doctor-evidence` agent through the Task tool. Do not invoke the globally installed `social-account-doctor` Skill; it is intentionally hidden from this session.
