...
try {
    ...
    if ($mode -eq 'create') {
        $result = Invoke-OpsManagerJson -Runtime $runtime -ManagerArguments @('create', '--name', $name, '--workspace-root', [string]$runtime.default_client_root_wsl, '--json')
    } elseif ($mode -eq 'attach') {
        $workspace = Read-Host '现有 WSL 绝对工作区路径'
        $result = Invoke-OpsManagerJson -Runtime $runtime -ManagerArguments @('attach', '--name', $name, '--workspace', $workspace, '--json')
