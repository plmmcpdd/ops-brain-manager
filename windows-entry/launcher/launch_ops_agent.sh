...
BASE_CLAUDE="/home/rong/.local/bin/claude"

fail() { printf 'Ops Brain Agent error: %s\n' "$1" >&2; exit "${2:-2}"; }
valid_client_id() { [[ "$1" =~ ^[A-Za-z0-9][A-Za-z0-9_-]*$ ]]; }
wait_for_file() {
...
clear_stale() {
  ...
  valid_client_id "$target" || fail 'invalid client_id for session state' 2
  lock="$STATE_ROOT/$target.lock"
...
valid_client_id "$CLIENT_ID" || fail 'invalid client_id' 2
...
mkdir -p -- "$STATE_ROOT"
LOCK_DIR="$STATE_ROOT/$CLIENT_ID.lock"
