# 运营大脑工作区管理器

这是一个本地客户 Ops Brain runtime 管理器。它管理客户登记、Cheat runtime 生命周期、启动 readiness 和会话 authority isolation；内容判断仍由原版 `cheat-on-content` 协议完成。

## 边界

- `.ops-brain/clients.json` 只保存客户显示名、稳定 ID、工作区路径、状态与来源等管理元数据，不保存内容业务数据。
- 每个客户工作区继续保存并运行原版 Cheat 数据；本工具不会读取或生成选题、评分、预测、Script、Hook、Rubric、Persona、Benchmark 或复盘数据。
- `/home/rong/tools/cheat-on-content` 是唯一的只读全局 Skill Runtime，不会被复制、克隆或绑定到任何客户。
- `cheat-on-content` 是 PRIMARY operational core；`social-account-doctor` 等 shared capabilities 只提供 evidence/diagnosis。
- 不提供数据库、Web UI、RBAC、自动发布或跨客户学习。

## WSL Ubuntu-E 使用

在本项目目录中运行：

```bash
python3 ops_brain.py create --name "客户 A" --workspace /home/rong/workspaces/client-a
python3 ops_brain.py attach --name "客户 B" --workspace /home/rong/workspaces/client-b
python3 ops_brain.py list
python3 ops_brain.py list --all
python3 ops_brain.py show --client client-a
python3 ops_brain.py open --client client-a
python3 ops_brain.py archive --client client-a
python3 ops_brain.py restore --client client-a
python3 ops_brain.py doctor
python3 ops_brain.py capabilities
python3 ops_brain.py runtime inventory
python3 ops_brain.py runtime status --client client-a
```

`create` 仍只创建空目录并登记。随后由 operator 显式提供原版 Cheat onboarding 选择并运行 `runtime initialize`；Manager 不猜客户内容形态，也不覆盖现有业务文件。

```bash
python3 ops_brain.py runtime initialize --client client-a \
  --content-form short-text --cadence-days 2 \
  --data-collection manual --pool-status none \
  --benchmark-status pending --hooks yes
```

标准 `open` 和 Windows launcher 只允许 `READY` runtime。`NOT_INITIALIZED`、`PARTIAL`、`INVALID`、`NEEDS_REPAIR` 都会显式失败，不会退化为普通 Claude 或 Doctor-first 会话。完整生命周期见 [`docs/CLIENT-RUNTIME-LIFECYCLE.md`](docs/CLIENT-RUNTIME-LIFECYCLE.md)。

`attach` 只登记一个已经存在的目录，绝不向该目录写入标记或内容。`archive` 只更新 Registry 状态，不移动或删除工作区；归档路径仍被保留，不能分配给其他客户。

客户工作区必须彼此独立：两个客户不能使用同一路径，也不能让任一路径成为另一工作区的父目录或子目录（包括符号链接解析后的真实路径）。`list --all` 用于同时查看 active 与 archived 客户。

客户 workspace 是 `.cheat-state.json`、rubric、prediction 和 retro 的唯一事实源；Manager 只实时验证，不复制业务 state。

`open` 先验证 runtime 为 `READY`。默认不会启动程序，只输出客户、绝对路径以及可复制命令；只有显式指定时才启动程序：

```bash
python3 ops_brain.py open --client client-a --app code
```

`doctor` 完全只读。退出码为：`0` 无问题、`1` 发现问题、`2` 无法安全读取 Registry。旧 Registry 中没有 `origin` 的客户会被标为 `legacy`，只提示人工确认，不会擅自推断或改写。

`capabilities` 独立、只读地检查 Claude 共享能力及其 provider 配置，不改变 `doctor` 的 Core/Registry 健康语义。安装和分发契约见 [`docs/SHARED-CAPABILITIES.md`](docs/SHARED-CAPABILITIES.md)。

## PublishOS 本地数据桥

Manager 仍不判断内容表现、不会修改 prediction 或 Rubric，也不会自动执行复盘。可选的 PublishOS bridge 只读取最小工作流元数据和云端只读 Performance API，并在客户工作区生成平台事实 `report.md` 与经过验证的标准化 JSON；这只是证据准备，不是运营复盘结论。

详细配置、数据边界、dry-run 和退出码见 [`docs/PUBLISHOS-LOCAL-BRIDGE.md`](docs/PUBLISHOS-LOCAL-BRIDGE.md)。

## Windows（次要示例）

在 PowerShell 中可使用：

```powershell
python .\ops_brain.py list
python .\ops_brain.py doctor
```

请优先在 WSL Ubuntu-E 中管理 WSL 路径。

`--registry <path>` 可用于测试或高级的本地登记册隔离；日常使用默认的 `.ops-brain/clients.json` 即可。

## Windows 桌面入口（Phase 1/2/3/5）

入口源位于 `windows-entry/`。在 Windows PowerShell 5.1 中显式部署到 `E:\运营大脑`：

```powershell
& "\\wsl.localhost\Ubuntu-E\home\rong\projects\content-ops-lab\ops-brain-manager\windows-entry\deploy_windows_entry.ps1" -SyncProjections
```

部署只创建 Windows 投影、配置和启动文件；真实客户工作区仍在 WSL。默认不自动启动 `claude-deepseek`，用户在 VS Code Remote WSL 工作区中手动运行它。

Windows Launcher 通过 ASCII 安全 JSON 调用 WSL Manager。机器接口可用于入口层：

```bash
python3 ops_brain.py list --all --json
python3 ops_brain.py show --client client-a --json
python3 ops_brain.py resolve-launch --client client-a --json
python3 ops_brain.py doctor --json
```

`create` 也支持由 Manager 生成默认客户目录名：

```bash
python3 ops_brain.py create --name "客户 A" --workspace-root /home/rong/projects/content-ops-clients
```

## 验证

```bash
python3 -m unittest discover -s tests -v
```
